...
exports.uploadMedia = async (req, res) => {
  try {
    const { type, caption, tripId } = req.body;
...
exports.getMedia = async (req, res) => {
  try {
    const media = await Media.findById(req.params.id).populate('userId', 'name email');
    if (!media) return res.status(404).json({ message: 'Media not found' });

    if (media.userId._id.toString() !== req.user._id.toString() && req.user.role !== 'admin') {
